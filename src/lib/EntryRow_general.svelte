...
<div class="table-row {hasPageInfo ? 'indent':''} entry-row {isIndented ? 'page-entry' : ''}"
    on:click={() => toggleEntryDetails(index)}
    on:keydown={(e) => handleKeyDown(e, index)}
    role="button"
    tabindex="0"
    class:selected={selectedEntryIndexes.has(index)}>
...
{#if selectedEntryIndexes.has(index)}
    <div class="detail-row {hasPageInfo ? 'indent':''}">
        <div class="custom-tabs">
            ...
            <button 
                class="tab-button" 
                class:active={selectedTabs.get(index) === 'Headers'}
                on:click={() => selectTab(index, 'Headers')}>
                Headers
            </button>
            {#if entry.requestQueryString.length > 0 || entry.requestPostData}
            <button 
                class="tab-button" 
                class:active={selectedTabs.get(index) === 'Payload'}
                on:click={() => selectTab(index, 'Payload')}>
                Payload
            </button>
            {/if}
            <!-- <button 
            ...
            <button 
                class="tab-button" 
                class:active={selectedTabs.get(index) === 'Timing'}
                on:click={() => selectTab(index, 'Timing')}>
                Timing
            </button>
            <button 
                class="tab-button" 
                class:active={selectedTabs.get(index) === 'Cookies'}
                on:click={() => selectTab(index, 'Cookies')}>
                Cookies [{entry.responseCookies.length}]
            </button>
            </div>
            <div class="tab-content">
            {#if selectedTabs.get(index) === 'Headers'}
                <!-- Headers content -->
            ...
            {:else if selectedTabs.get(index) === 'Payload'}
                <!-- Payload content -->
            ...
            {:else if selectedTabs.get(index) === 'Response'}
                <!-- Response content -->
            {:else if selectedTabs.get(index) === 'Initiator' && entry._initiator}
                <!-- Initiator content -->
            {:else if selectedTabs.get(index) === 'Timing'}
                <!-- Timing content -->
            ...
            {:else if selectedTabs.get(index) === 'Cookies'}
                <!-- Cookies content -->
